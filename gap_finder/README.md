...
## status_check.py - Helper Program
...
In the future, this will be modified to check for a fully completed status. If found, the gap log file will updated to say that the playback is complete. It is possible that this feature may be placed in `data_check.py` instead so that verification can be performed. Alternatively, this could just set the status to complete (e.g. `'test': COMPLETE`) and then `data_check.py` could have a HITL prompt/response that could mark the gap as verified (e.g. `'test': VERIFIED`).



## data_check.py - Helper Program

### Description
The `data_check.py` tool does as the name suggests and helps with data verification and availability checks after a playback has completed. The tool reads in a gap log file generated by starting a playback using `rsn_gaps.py` and then creates one plot for each gap. Each plot spans the time period of the gap (i.e. `gap.start` to `gap.end`). The "test" variable defined in `rsn-tools.core.streams.rdList` is used for plotting. Data are retrieved via M2M from the server specified by the server parameter in the gap log file.

Plots are displayed one at a time. Once a plot is closed, the query for the next plot's data will be sent and the new plot will be generated, and so on.

## Typical use Case
The only use case is to use `data_check.py` to verify data availability and data gross range QC after a playback started by `rsn_gaps.py`. Here's how
```bash
[user@test]: cd /path/to/rsn_tools/gap_finder/
[user@test]: python data_check.py rsn_gaplist_2020-03-03T19_52Z.log
```
This will spit out a plot. Check the plot for any unexpected data gaps. Make sure data span the entire plot if that is the expected instrument behavior, and also look for any strange spikes or outliers.

### Other Options
At present, there are no additional options for the status check tool.

### The Future and Beyond
See the future and beyond section for `status_check.py`.
